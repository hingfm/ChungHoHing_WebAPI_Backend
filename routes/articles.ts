...
const deleteArticle = async (ctx: RouterContext, next: any) => {
  ...
  */
  ...
  const found: any = await model.getById(id);
  if (found && found.region == user.region) {
...
router.get("/stat", async (ctx: RouterContext, next) => {
  const all = await model.getAll(999, 1, "dateCreated", "ASC")
  let kindStat: any[] = []
  let regionStat: any[] = []
  all.forEach((a: any) => {
    let region = a.region || "Unknown"
    let regionItem = regionStat.find(i => i.region == region)
    if (regionItem) {
      regionItem.count += 1
    } else {
      regionStat.push({region: region, count: 1})
    }
    let kind = a.kinds || "Unknown"
    let kindItem = kindStat.find(i => i.kind == kind)
    if (kindItem) {
      kindItem.count += 1
    } else {
      kindStat.push({kind: kind, count: 1})
    }
  })
  ctx.body = {
    kindStat,
